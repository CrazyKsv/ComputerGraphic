...
struct ray3D{
	...
	int goInside; //1 from air to obj, 0 from obj to air
};
...
struct object3D{
	...
	// functions to return a random light source if object is light source
	void (*lsRandomRay)(struct object3D *plane, struct ray3D *ray, struct point3D *c);
